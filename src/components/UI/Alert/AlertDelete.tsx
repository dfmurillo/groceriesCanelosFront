...
const AlertDelete = forwardRef<AlertDeleteFunctionsType, AlertDeletePropsType>(({ handleAction, alertText }, ref) => {
  ...
  return (
    <>
      {visible && (
        <div role='alert' className='alert alert-warning' data-testid='alert-delete'>
          <svg
          ...
          <button onClick={() => setVisible(false)} className='btn btn-sm' data-testid='alert-cancel'>
            Cancel
          </button>
          <button onClick={() => handleAction()} className='btn btn-error btn-sm' data-testid='alert-confirm'>
            I&apos;m sure
          </button>
        </div>
      )}
